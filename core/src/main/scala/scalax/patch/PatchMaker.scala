...
object PatchMaker extends LowPriorityPatchMaker {
  ...
  private[patch] case class ConstantPatchMaker[T]()
  ...
  private[patch] case class ArithmeticPatchMaker[T, D]()(implicit num: ArithmeticAdapter.Aux[T, D])
      extends AbstractPatchMaker[T](
        Mono,
        {
          case (l, r) if l == r                     => Empty
  ...
  private[patch] case class Sum1PatchMaker[F[_], T]()(implicit sum1: Sum1Adapter[F, T], pm: PatchMaker[T])
  ...
  private[patch] case class Sum2PatchMaker[F[_, _], L, R]()(implicit
  ...
  private[patch] case class UnorderedPatchMaker[F[_], T]()(implicit coll: UnorderedCollectionAdapter[F, T])
      extends AbstractPatchMaker[F[T]](
        UnorderedCollection,
        { case (l, r) =>
          import coll._
  ...
  private[patch] case class OrderedPatchMaker[F[_], T]()(implicit coll: OrderedCollectionAdapter[F, T])
      extends AbstractPatchMaker[F[T]](
        OrderedCollection,
        { case (l, r) =>
          import coll._
  ...
  private[patch] case class IndexedPatchMaker[F[_], T]()(implicit coll: IndexedCollectionAdapter[F, T])
      extends AbstractPatchMaker[F[T]](
        IndexedCollection,
        { case (i, f) =>
          import coll._
  ...
  private[patch] case class KeyedPatchMaker[F[_, _], K, V]()(implicit coll: KeyedCollectionAdapter[F, K, V])
