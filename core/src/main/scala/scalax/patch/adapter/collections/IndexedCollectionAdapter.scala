...
trait IndexedCollectionAdapter[F[_], V] { self =>
  ...
  class IndexedOps(coll: F[V]) {
    ...
    def updatedWith(delta: Map[Int, Patch[V]]): F[V] = delta.foldLeft(coll) { case (coll, (key, patch)) =>
      self.applyPatch(coll, key, patch)
    }
...
object IndexedCollectionAdapter extends ScalaVersionSpecificIndexedCollectionAdapter {
  ...
  implicit def forArray[T](implicit compT: PatchMaker[T]): IndexedCollectionAdapter[Array, T] =
    new IndexedCollectionAdapter[Array, T] {
