...
sealed trait KeyedCollectionAdapter[F[_, _], K, V] {
...
object KeyedCollectionAdapter {

  implicit def forMap[F[A, B] <: Map[A, B], K, V](implicit compV: PatchMaker[V], cc: Factory[(K, V), F[K, V]]): KeyedCollectionAdapter[F, K, V] = new KeyedCollectionAdapter[F, K, V] {
    ...
    override def diff(left: F[K, V], right: F[K, V]): Map[K, Patch[V]] = {
      ...
      var updated = (right.keySet intersect left.keySet).foldLeft[Map[K, Patch[V]]](Map.empty) { case (agg, commonKey) =>
