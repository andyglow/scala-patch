...
trait ScalaVersionSpecificKeyedCollectionAdapter {
  ...
  implicit def forMap[F[A, B] <: Map[A, B], K, V](implicit compV: PatchMaker[V], cc: CanBuildFrom[Map[K, V], (K, V), F[K, V]]): KeyedCollectionAdapter[F, K, V] = new KeyedCollectionAdapter[F, K, V] {
