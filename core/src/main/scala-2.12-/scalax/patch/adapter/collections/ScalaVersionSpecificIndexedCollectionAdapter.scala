...
trait ScalaVersionSpecificIndexedCollectionAdapter {
  ...
  implicit def forIndexedSeq[F[X] <: IndexedSeq[X], T](implicit compT: PatchMaker[T], cc: CanBuildFrom[IndexedSeq[T], T, F[T]]): IndexedCollectionAdapter[F, T] = new IndexedCollectionAdapter[F, T] {
    ...
    override def resize(coll: F[T], sizeDelta: Int): F[T] = {
      if (sizeDelta < 0) {
        ...
        (0 until sizeDelta).foldLeft(coll) {
          case (coll, _) => cc(coll :+ null.asInstanceOf[T]).result()
        }
      } else
        coll
    }
