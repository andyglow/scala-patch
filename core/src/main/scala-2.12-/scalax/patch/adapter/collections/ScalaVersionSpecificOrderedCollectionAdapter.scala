...
trait ScalaVersionSpecificOrderedCollectionAdapter {
  ...
  implicit def forLinearSeq[F[X] <: LinearSeq[X], T: PatchMaker](implicit
    cc: CanBuildFrom[Traversable[T], T, F[T]]
  ): OrderedCollectionAdapter[F, T] = new OrderedCollectionAdapter[F, T] {
    import Diff._
    import Evt._
    ...
    override def apply(coll: F[T], diff: Diff[T]): F[T] = {
      ...
      diff.events foreach {
        ...
        case Upgrade(es) =>
          res ++= es.zip(tail.take(es.length)).map { case (patch, e) => patch(e) }; tail = tail.drop(es.length)
      }
